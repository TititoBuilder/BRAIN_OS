---
tags: [trigger, manual, brain_os]
type: MANUAL
project: BRAIN_OS
fires_when: session_close.py manual run
implemented_by: 09_TOOLS/session_close.py
---
# TRIGGER: Session close
## What fires it
`session_close.py` manual run from `C:\Dev\Projects\soccer-content-generator\`
File: `C:\BRAIN_OS\09_TOOLS\session_close.py`
## Activates
[[KNOWLEDGE_INGESTION_PROTOCOL_V2]]
## Project
[[BRAIN_OS]]
## CLAUDE.md
`C:\BRAIN_OS\CLAUDE.md`
## Connected to
[[Trigger_Architecture]]
[[Trigger_Daily_Log_Update]]
[[Trigger_New_Idea]]
[[Master_Control]]
