...
holds markdown reference sheets for every external API in use ([[Anthropic_Claude]],
[[twitter-api]], [[Telegram_Bot]], [[Football_API]], etc.) and `claude_monitor.py`, a script that
pulls live usage data from the Anthropic Admin API and fires [[Telegram_Bot]] alerts
...
| Env | `python-dotenv` |
| Alerts | `urllib.request` → [[Telegram_Bot]] Bot API |
...
`claude_monitor.py` does not generate AI content — it queries the [[Anthropic_Claude]]
...
- **[[Telegram_Bot]]**: all alerts use `TELEGRAM_BOT_TOKEN` + `TELEGRAM_CHAT_ID` via `urllib.request` POST to `api.telegram.org`. No other notification service.
...
- **claude_monitor.py** — [[Anthropic_Claude]] Admin API cost monitor + [[Telegram_Bot]] budget alerts
- **[[Telegram_Bot]]** — alert delivery (same bot token as BDF pipeline)
...
All alerts from `claude_monitor.py` route through the existing [[Telegram_Bot]].
...
|---|---|
| `claude_monitor.py` in a loop / cron without reading output | It makes live [[Anthropic_Claude]] Admin API calls that count toward rate limits; always review output before scheduling |
...
| File | Purpose |
|---|---|
| `claude_monitor.py` | Live Claude API cost monitor + [[Telegram_Bot]] budget alerts |
| [[Anthropic_Claude]] | Anthropic API reference (models, pricing, limits) |
| [[Football_API]] | API-Football / RapidAPI reference |
| [[GoogleDrive_API]] | Google Drive API reference |
| [[KlingAI]] | Kling AI video generation API reference |
| [[NewsAPI]] | NewsAPI.org reference |
| [[OpenAI_gpt-image-1]] | OpenAI image generation API reference |
| [[Pixabay_API]] | Pixabay stock image API reference |
| [[Telegram_Bot]] | Telegram Bot API reference (our bot setup, commands) |
| [[twitter-api]] | Twitter / X API v2 reference (OAuth 2.0, endpoints) |
...
Output: terminal table grouped by model + [[Telegram_Bot]] alert if over budget.
