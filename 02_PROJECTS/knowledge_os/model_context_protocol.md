---
knowledge_os_machine_key: model_context_protocol
knowledge_os_domain: AI/ML
knowledge_os_status: Mastered
knowledge_os_score: 92
knowledge_os_priority: High
knowledge_os_evidence: 6 MCP servers operational, resolve+obsidian+context7
...
executes, and closes with no persistent pool. Claude Code reads MCP server
configuration from settings.json at startup. A critical naming distinction
applies: the Obsidian package is obsidian-mcp, not obs-mcp. The prefix obs
belongs to OBS Studio, the streaming software. The wrong package name causes
silent failure with no useful error at connection time.
