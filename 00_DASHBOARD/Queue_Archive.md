# BRAIN_OS — Queue Archive

Completed items moved from Queue.md. Oldest first.

---

## Completed — prior

- [x] Session 2: Apply Knowledge Graphs Over Lists to 02_AGENTS
- [x] Update calendar events (DALL-E 3 deadline May 12 cleaned up)

---

## Completed — 2026-05-28

- [x] Knowledge OS Phase 1-3 (encyclopedia, stitcher, obsidian sync)
- [x] Knowledge OS Drive structure (28 folders, domain isolation)
- [x] Knowledge OS user manual
- [x] drive_index.json wired — 25 topics audio-linked
- [x] drive_index fix: file IDs for BRAIN_OS_Vault entries
- [x] 22 vault audio nodes added to dropdown
- [x] read_along_app_session audio node added
- [x] Read-Along App full architecture docs (4-tab interface)
- [x] Whisper GPU analysis (42x realtime, Triton fallback)
- [x] RAG chapter (Cristian's build-first explanation)
- [x] Dashboard UI fixes (header visibility, button hover states)
- [x] Focus Now cards clickable (edit modal)
- [x] brain-audio steganographic fingerprint module
- [x] book-compiler fingerprint integration (auto + post-stitch verify)
- [x] Obsidian graph colors fixed (UTF-8 BOM root cause)
- [x] restore_graph_colors.ps1 built + committed
- [x] BRAIN_OS duplicate file audit + cleanup
- [x] BRAIN_OS principle: never start without feeding context
- [x] .env secrets purged from 117 commits
- [x] GitHub PAT rotated
- [x] Excluded audio_staging binaries from git
- [x] Admin key file removed from tracking
- [x] Cristian_Principles.md: PowerShell Encoding rule added
- [x] Cristian_Principles.md: Documentation Must Reflect Reality added
- [x] Cristian_Principles.md: Knowledge Graphs Over Lists added
- [x] Trigger_Architecture.md: 13-trigger master index built
- [x] 08_TRIGGERS: all 13 trigger files created/rewritten with full bidirectional links
- [x] 3 Q&A notes ingested (2026-05-20)
- [x] soccer-content-generator: session close + brain sync 2026-05-27

---

## Completed — 2026-06-11

- [x] Author + voice 3 topics: edge_tts, kokoro_tts, message_queues (live in LISTEN tab)

---

## Completed — 2026-06-16

- [x] Gold Capstone: 9-lesson course authored, voiced, indexed (id: format), learning path added, committed + deployed (BRAIN_OS a2ea9b8, read-along 9abc2fe)
- [x] session_close.py --project flag docstring/parser drift — FIXED
- [x] read-along-app CLAUDE.md DRIVE_INDEX_JSON drift — FIXED + committed (ad6c125)
- [x] id: root-cause fix in populate_staging.py + Railway DRIVE_INDEX_JSON shadow deleted
- [x] session_close.py "make git add surgical" — determined STALE (no git add in file); closed
- [x] "Convert path-format index entries to id:" — superseded; replaced by Borrowed-audio RE-VOICE worklist (blind conversion locks in wrong audio)


<!-- auto-ingested 2026-08-13 -->
## 2026-08-13
- Direction 1 COMPLETE: server.py read, architecture mapped
- Direction 2 COMPLETE
- Direction 6 COMPLETE: gig_tracker imports
- Direction 7b COMPLETE
- Direction 7c COMPLETE
- Direction 7d COMPLETE
- Direction 7e IN PROGRESS: van_build rename + is_onetime confirmed
- 43 files committed from Direction 4 Downloads triage
- 11 Obsidian vault docs populated for gig_tracker project
- FLAGS_MANUAL and LESSON_QUEUE added to vault
- Flag deduplication and sort finalized


<!-- auto-ingested 2026-08-14 -->
## Directions 7b–7e (2026-08-14)
- 7b: COMPLETE
- 7c: COMPLETE
- 7d: COMPLETE
- 7e: IN PROGRESS → queued as Direction 8
- van_build rename + is_onetime field confirmed
- PathConfig canonical pattern documented


<!-- auto-ingested 2026-08-15 -->
- 2026-08-15 17:09 | Projects: BDF, CA, BRAIN_OS | Directions 7b–7e complete, CristianConstruction docs added, soccer-content-generator avatar pipeline patched, gig_tracker flags committed, lesson_10 added.
